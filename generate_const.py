...
def main(args):
    ...
    for data in tqdm(dataloader, ncols=100):
        ...
        z = torch.randn(label.size(0), label.size(1),
                        ...
                        device=device)
        bbox_init = netG(z, label, padding_mask)
        ...
        input_bbox = data.edge_stores[0]['x'][1:, :]
        input_label = data.y[1:]  # check labels
        for j in range(bbox.size(0)):
            ...
            if len(results) < args.num_save:
                out_path = out_dir / f'input_{len(results)}.png'
                convert_layout_to_image(
                    input_bbox,
...
if __name__ == '__main__':
    ...
    sys.argv = ["generate_const.py", "pretrained/layoutganpp_rico.pth.tar", "--const_type", "beautify", "--out_path", "output/beautify/generated_layouts.pkl", "--num_save", "6"]
